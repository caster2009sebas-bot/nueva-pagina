document.addEventListener('DOMContentLoaded', () => {
    ...
    calculateButton.addEventListener('click', async function() {
        ...
        try {
            ...
            resultContainer.innerHTML = `
                <div class="p-5 bg-cyan-900 border-l-4 border-cyan-700 rounded-lg shadow-2xl text-left">
                    ...
                    <div class="space-y-2 text-gray-200">
                        <p class="flex justify-between">Monto Original: <span class="font-semibold text-white">$${amount.toFixed(2)}</span></p>
                        <p class="flex justify-between">Propina (10%): <span class="font-semibold text-cyan-400">$${tipAmount.toFixed(2)}</span></p>
                    </div>
                    <div class="mt-4 pt-3 border-t border-gray-700 flex justify-between items-center">
                        ...
                        <p class="text-3xl font-extrabold text-cyan-400">$${totalAmount.toFixed(2)}</p>
                    </div>
                </div>
            ...
            resultContainer.innerHTML = '<div class="p-4 bg-red-900 border border-red-700 text-red-300 rounded-lg text-center font-medium">Error interno al realizar el cálculo.</div>';
            payButton.style.display = 'none';
            calculateButton.style.display = 'block';
        }
    });
